...
namespace fastJSON
{
    ...
    public class deserializer
    {
        ...
        {
            Type type = typeof(T);
            return (T)ChangeType1(o, type);
        }

        static object ChangeType1(object o,Type type)
        {
            //if (this._params.UseExtensions && !this._params.UsingGlobalTypes && o != null && o is Dictionary<string, object>)
            //{
            //    Dictionary<string, object> oo = (Dictionary<string, object>)o;
            //    if(oo.ContainsKey("$type"))
            //    {
            //        object oType = oo["$type"];
            //        type = Type.GetType(oType.ToString());
            //    }
            //}
            ...
            else if (type.IsPrimitive || type == typeof(string) || type.IsEnum)  //基本类型
            {
            ...
            {
                if (type.GetGenericTypeDefinition() == typeof(Dictionary<,>) & o is Dictionary<string, object>)
                    return o;
                else if(type.GetGenericTypeDefinition() == typeof(List<>) & o is List<object>)
                {
                    return o;
                }
            }

...
namespace CppConverter
{
    ...
    class Model
    {
        ...
        static HashSet<string> LoadFailed = new HashSet<string>();
        ...
        public static Metadata.DB_Type GetType(string full_name)
        {
            ...
            //if(!LoadFailed.Contains(full_name))
            //{
            //    LoadFailed.Add(full_name);
    ...
    class Program
    {
        ...
        //        return Model.Instance.GetIndifierInfo(e.Name).type;
        //    }
        ...
        //    {
        //        return Model.GetType(Model.currentType.base_type);
        //    }
        ...
        static void ConvertType(Metadata.DB_Type type)
        {
            ...
            //头文件
            {
                ...
                sb.AppendLine(string.Format("namespace {0}{{", type._namespace));
                {
                    ...
                    if(configs.ContainsKey(type.static_full_name))
                    {
                        if(!string.IsNullOrEmpty( configs[type.static_full_name].ext_header))
                        {
                            AppendLine("#include \"" + configs[type.static_full_name].ext_header +"\"");
                        }
                    }
            ...
            //cpp文件
            {
                ...
                if(!type.is_enum && !type.is_generic_type_definition)
                {
                    ...
                    if (configs.ContainsKey(type.static_full_name))
                    {
                        if (!string.IsNullOrEmpty(configs[type.static_full_name].ext_cpp))
                        {
                            AppendLine("#include \"" + configs[type.static_full_name].ext_cpp + "\"");
                        }
                    }
        ...
        static void ConvertMemberHeader(Metadata.DB_Member member)
        {
            ...
            {
                Model.Instance.EnterMethod(member);
                AppendLine(GetModifierString(member.modifier) + ":");
        ...
        static void ConvertStatement(Metadata.DB_BlockSyntax bs)
        {
            AppendLine("{");
            depth++;
            Model.Instance.EnterBlock();
            foreach(var s in bs.List)
            {
                ConvertStatement(s);
            }
            depth--;
            Model.Instance.LeaveBlock();
            AppendLine("}");
        }
        ...
        static void ConvertStatement(Metadata.DB_LocalDeclarationStatementSyntax bs)
        {
            ...
            for (int i=0;i<bs.Declaration.Variables.Count;i++)
            {
                ...
                Model.Instance.AddLocal(bs.Declaration.Variables[i].Identifier, Model.GetType(bs.Declaration.Type));
            }
            sb.AppendLine(";");
        }
        ...
        static string ExpressionToString(Metadata.Expression.MethodExp es)
        {
            ...
            if (es.Caller is Metadata.Expression.IndifierExp)
            {
                ...
                Metadata.Model.IndifierInfo ii = Model.Instance.GetIndifierInfo(ie.Name);
        ...
        static string ExpressionToString(Metadata.VariableDeclarationSyntax es)
        {
            ...
            {
                Model.Instance.AddLocal(es.Variables[i].Identifier, Model.GetType(es.Type));
